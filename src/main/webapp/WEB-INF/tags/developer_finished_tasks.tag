...
<spring:message code="developer.home.page.pending.table.task" text="Task" var="finishedTaskMessage"/>
<spring:message code="developer.home.page.finished.table.estimate.used" text="Estimate/Used" var="finishedEstimateUsedMessage"/>
<spring:message code="developer.home.page.pending.table.executor" text="Executor" var="finishedExecutorMessage"/>
<spring:message code="developer.home.page.finished.table.finished.time" text="Finished Time" var="finishedTimeMessage"/>
...
<displaytag:table list="${developerOverviewDto}" class="table table-striped table-hover" id="task" form="developerForm">
    ...
    <displaytag:column title="${finishedTaskMessage}" headerClass="width40">
        <div class="dropdown">
            ...
            <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
                <custom:finished_task_actions currentSprint="${developerOverviewDto.currentSprint}" task="${task}"/>
            </ul>
        </div>
    </displaytag:column>
    <displaytag:column title="${finishedEstimateUsedMessage}">
        ${task.estimateTime} / <span class="text-info">${task.actualUsedTime}${task.timeDifferenceAsHtml}</span>
    </displaytag:column>
    <displaytag:column property="executor.showName" title="${finishedExecutorMessage}"/>
    <%--<displaytag:column property="priority.label" title="Priority"/>--%>
    <displaytag:column property="finishTime" title="${finishedTimeMessage}"/>
...
<div class="badge badge-info developer-time-budget pull-right">
    <spring:message code="developer.home.page.pending.total" text="Total"/>:
    <strong>${developerOverviewDto.totalEstimatedHours}</strong> /
    <strong>${developerOverviewDto.totalUsedHours}</strong>
